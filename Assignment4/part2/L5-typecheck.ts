...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, LetValuesExp, CExp, isValuesExp, ValuesExp, isLetValuesExp } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, makeNonEmptyTupleTExp, makeEmptyTupleTExp } from "./TExp";
...
export const typeofExp = (exp: Parsed, tenv: TEnv): Result<TExp> =>
    ...
    isValuesExp(exp) ? typeofValuesExp(exp, tenv) :
    isLetValuesExp(exp) ? typeofLetValuesExp(exp, tenv) :
...
export const typeofValuesExp = (exp: ValuesExp, tenv: TEnv): Result<TExp> => {
    const typeResArr : Result<TExp>[] = map((val: CExp) => typeofExp(val, tenv), exp.vars);
    const resArr = mapResult((a) => a , typeResArr);
    return isEmpty(exp.vars) ? makeOk(makeEmptyTupleTExp()) : 
        bind(resArr, (texps) => makeOk(makeNonEmptyTupleTExp(texps)));
};

export const typeofLetValuesExp = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
    let valsVals : CExp[] = [];
    const vars = flatten(map((b) => b.var, exp.vbindings));
    const varsvals = (map((v) => v.var, vars));
    let vals = map((b) => b.val, exp.vbindings);
    !allT(isValuesExp, vals) ? makeFailure("mismayching types") : valsVals = flatten(map((v) => v.vars, vals));
    const varTEs = map((v) => v.texp, vars);

    const constraints = zipWithResult((varTE, val) => 
                                bind(typeofExp(val, tenv),
                                    (typeOfVal: TExp) => checkEqualType(varTE, typeOfVal, exp)), varTEs, valsVals);
    return bind(constraints, _ => typeofExps(exp.body, makeExtendTEnv(varsvals, varTEs, tenv)));
};