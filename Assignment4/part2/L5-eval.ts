...
import { CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, isCExp, ValuesExp, LetValuesExp, isValuesExp, isLetValuesExp } from './L5-ast';
...
import { isClosure, makeClosure, Closure, Value, makeTuple } from "./L5-value";
...
const evalSet = (exp: SetExp, env: Env): Result<void> =>
    safe2((val: Value, bdg: FBinding) => makeOk(setFBinding(bdg, val)))
        ...
        bind(mapResult((v : CExp) => applicativeEval(v, env), exp.vars), (values) => makeOk(makeTuple(values)))

// LET: Direct evaluation rule without syntax expansion
// compute the values, extend the env, eval the body.
const evalLetValuesExp = (exp: LetValuesExp, env: Env): Result<Value> => {
    const vals = mapResult((v : CExp) => applicativeEval(v, env), exp.vbindings.map((v) => v.val));
    const vars = exp.vbindings.map((b) => b.var).map((vda)=> vda.map((vd) => vd.var));
    const varsAndVals : Result<[string[], Value][]> = bind(vals, (vals:Value[]) => makeOk(zipWith((x:string[],y:Value) => [x,y] , vars, vals)))
    const EEnv = bind(varsAndVals, (arr) => makeOk(arr.reduce((acc,curr) => makeExtEnv( curr[0], [curr[1]], acc) , env)))
    return bind(EEnv, (Eenv) => evalSequence(exp.body, Eenv));
}