...
export const expComponents = (e: Exp): CExp[] =>
    ...
    isSetExp(e) ? [e.val] :
    isDefineExp(e) ? [e.val] :
    isValuesExp(e) ? e.vars :
    isLetValuesExp(e) ? [...e.body, ...map((b) => b.val, e.vbindings)] :
...
export interface ValuesExp {tag: "ValuesExp"; vars: CExp[]; }
export const makeValuesExp = (v: CExp[]): ValuesExp =>
    ({tag: "ValuesExp", vars: v});
export const isValuesExp = (x: any):x is ValuesExp => x.tag === "ValuesExp";

export interface LetValuesExp {tag: "LetValuesExp"; vbindings: BindingValues[]; body: CExp[]}
export const makeLetValuesExp = (vbind: BindingValues[], body: CExp[]): LetValuesExp =>
    ({tag: "LetValuesExp", vbindings: vbind, body: body});
...
export const parseL5CompoundCExp = (op: Sexp, params: Sexp[]): Result<CExp> => {
    return isString(op) && isSpecialForm(op) ? parseL5SpecialForm(op, params) :
    parseAppExp(op, params);
}

export const parseL5SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    isEmpty(params) && op !== "values" ? makeFailure("Empty args for special form") :
...
const parseValuesExp = (params: Sexp[]): Result<ValuesExp> =>
    isEmpty(params) ? makeOk(makeValuesExp([])) :
        bind(mapResult(parseL5CExp, params),
            (params: CExp[]) => makeOk(makeValuesExp(params)));

const parseLetValuesExp = (bindingsV: Sexp, body: Sexp[]): Result<LetValuesExp> =>
    isEmpty(body) ? makeFailure('Body of "let-values" cannot be empty') :
    ! isGoodBindings(bindingsV) ? makeFailure(`Invalid bindings: ${JSON.stringify(bindingsV)}`) :
    safe2((bdgs: BindingValues[], body: CExp[]) => makeOk(makeLetValuesExp(bdgs, body)))
        (parseBindingsValues(bindingsV), mapResult(parseL5CExp, body));

const parseBindingsValues = (bindingsV: [Sexp, Sexp][]): Result<BindingValues[]> =>{
    let y = map(b => b[1], bindingsV)
    let x = mapResult(parseL5CExps, map(b => b[1], bindingsV))
    return safe2((vds: VarDecl[][], vals: CExp[]) => makeOk(zipWith(makeBindingValues, vds, vals)))
        (mapResult(parseVarDecls, map(b => b[0], bindingsV)), mapResult(parseL5CExp, map(b => b[1], bindingsV)));
}
const parseL5CExps = (se:Sexp) : Result<CExp[]> =>{
    return isCompoundSexp(se) ? mapResult(parseL5CExp, se) :
        bind(parseL5CExp(se), (ce) => makeOk([ce]))
}

const parseVarDecls = (se:Sexp) : Result<VarDecl[]> =>
    isCompoundSexp(se) ? mapResult(parseVarDecl, se) :
        bind(parseVarDecl(se), (vd) => makeOk([vd]))
...
const unparseValuesExp = (ve: ValuesExp): Result<string> =>
    bind(mapResult(unparse, ve.vars), (vals: string[]) => makeOk(`(values ${join(" ", vals)})`));

const unparseBindingsValues = (bindingsV: BindingValues[]): Result<string> =>
    bind(mapResult(bdg => safe2((vd: string[], val: string) => makeOk(`((${join(" ", vd)}) ${val})`))(mapResult((vars) => unparseVarDecl(vars),bdg.var), 
                                        unparse(bdg.val)), bindingsV),
         (bdgs: string[]) => makeOk(join(" ", bdgs)));

const unparseLetValuesExp = (lve: LetValuesExp): Result<string> =>
    safe2((bdgs: string, body: string) => makeOk(`(let-values (${bdgs}) ${body})`))
        (unparseBindingsValues(lve.vbindings), unparseLExps(lve.body));
