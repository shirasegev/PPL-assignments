...
import { Value, isTuple } from './L5-value';
...
export const makeExtEnv = (vs: string[], vals: Value[], env: Env): ExtEnv =>
    ({tag: "ExtEnv", frame: isUndefined(vals[0]) ? makeFrame(vs, vals) : isTuple(vals[0]) ? makeFrame(vs, vals[0].val) : makeFrame(vs, vals), env: env});
