...
export type SExpValue = number | boolean | string | PrimOp | Closure | SymbolSExp | EmptySExp | CompoundSExp | Tuple | void;
...
export const makeTuple = (v: SExpValue[]): Tuple => ({tag: "Tuple", val: v});
...
export const valueToString = (val: Value): string =>
    ...
    isTuple(val) ? `(values ${join(" ", val.val)})` :
    isEmptySExp(val) ? "'()" :
