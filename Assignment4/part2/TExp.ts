...
const parseCompoundTExp = (texps: Sexp[]): Result<CompoundTExp> => {
    ...
    return (pos === -1) ? parseTupleTExp(texps):                          // No arrow
           ...
           safe2((args: TExp[], returnTE: TExp) => makeOk(makeProcTExp(args, returnTE)))
                (parseTExps(texps.slice(0, pos)), parseTExp(texps[pos + 1]));
};

const parseTExps = (sexps: Sexp[]): Result<TExp[]> => {
    ...
    return isEmptyTuple(sexps) ? makeOk([]) : bind(splitEvenOdds(sexps),
...
;; Or: Empty
*/
const parseTupleTExp = (sexps: Sexp[]): Result<TupleTExp> => {
    const isEmptyTuple = (sexps: Sexp[]): boolean =>
        (sexps.length === 1) && (sexps[0] === 'Empty');
    // [x1 * x2 * ... * xn] => [x1,...,xn]
    const splitEvenOdds = (texps: Sexp[]): Result<Sexp[]> =>
    isEmpty(texps) ? makeOk([]) :
    isEmpty(rest(texps)) ? makeOk(texps) :
    ...
    return isEmptyTuple(sexps) ? makeOk(makeEmptyTupleTExp()) : 
                bind(splitEvenOdds(sexps),
                    (argTEs: Sexp[]) => bind(mapResult(parseTExp, argTEs), (texps: TExp[]) => 
                    texps.map((te) => isTupleTExp(te)).every((v) => v === false) ?
                        makeOk(makeNonEmptyTupleTExp(texps)) : makeFailure("Tuple inside Tuple")))
}
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isTupleTExp(x) ? isEmptyTupleTExp(x) ? makeOk('(Empty)') :
            safe2((paramTE: string, paramTEs: string[]) => makeOk([paramTE].concat(chain(te => ['*', te], paramTEs))))
                            (unparseTExp(first(x.TEs)), mapResult(unparseTExp, rest(x.TEs))) :
