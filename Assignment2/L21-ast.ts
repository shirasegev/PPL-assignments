...
export const makeForExp = (v: VarDecl, start : NumExp, end: NumExp, body: CExp): ForExp => 
    ({tag: "ForExp", var : v, start : start, end: end, body: body});
...
import { isEmptyEnv } from '../imp/L3-env';
...
/*
Purpose: Given an L21 program string the procedure returns its parsed AST
Signature: parseForExp(Sexp, Sexp[])
Type: [Sexp * Sexp[]] => [Result<ForExp>]
*/
// <ForExp> -> (for <VarDecl> <NumExp> <NumExp> <CExp>)
const parseForExp = (v: Sexp, body: Sexp[]): Result<ForExp> => 
    body.length !== 3 || !isIdentifier(v) ? makeFailure("Invalid vars for ForExp") :
    bind(mapResult(parseL21CExp, body), 
        (cexps: CExp[]) => isNumExp(cexps[0]) && isNumExp(cexps[1]) ?
        ((cexps[0].val > cexps[1].val) || isEmpty(cexps[2])) ? makeFailure("Range is invalid") :
        makeOk(makeForExp(makeVarDecl(v), cexps[0], cexps[1], cexps[2])) :
