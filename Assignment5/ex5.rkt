...
(define sqrt-lzl 
  (lambda (x init)
   (sqrt-iter-lzl x init)
  )
)

(define sqrt-iter-lzl
  (lambda (x guess)
     (cons-lzl (cons guess (abs (- (square guess) x))) 
        (lambda () (sqrt-iter-lzl x (improve guess x))))))
...
;;Purpose: return approximation of the square root of the given number x, according to Newton method, starting from init guess with epsilon threshold. 
;; The procedure uses sqrt-lzl and find-first procedures.
;;Type: [Number * Number * Number -> Number]
;;Pre-condition: init =/= 0
;;Tests: (sqrt2 2 1 0.0001) → 1 169/408
(define sqrt2
  (lambda (x init epsilon)
    (car (find-first (sqrt-lzl x init) (lambda (y) (< (cdr y) epsilon))))
  )
)
...
(define collect-all-values-2
 (lambda (lists key)
    (if (empty? lists)
      '()
      (if (eq? (get-value$ (car lists) key (lambda (x) x) (lambda () '())) '())
        (collect-all-values-2 (cdr lists) key)
        (cons (get-value$ (car lists) key (lambda (x) x) (lambda () '())) (collect-all-values-2 (cdr lists) key))
      )
    )
  )
)
