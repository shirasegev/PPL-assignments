...
export const makeExtEnv = (vs: string[], vals: CExp[], curr: Env, env: Env): ExtEnv =>
    ({tag: "ExtEnv", vars: vs, vals: vals, curr: curr, nextEnv: env});
...
const applyExtEnv = (env: ExtEnv, v: string): Result<[CExp, Env]> =>
    env.vars.includes(v) ? makeOk([env.vals[env.vars.indexOf(v)],env.curr]) :
