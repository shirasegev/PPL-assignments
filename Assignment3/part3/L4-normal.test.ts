...
describe('L4 Normal Eval', () => {
    ...
    it('additional tests, as part of 3.3', () => {
        // 1. Test with infinite loop
        expect(bind(parseL4(`
        (L4 (define inf_loop (lambda (x) (inf_loop x)))
        (define g (lambda (x y)
                    (if (= x 0)
                        0
                        y)))
        (g 0 (inf_loop 0)))`), evalNormalProgram)).to.deep.equal(makeOk(0));
        // 2. Test with exceptions
        expect(bind(parseL4(`
            (L4 (define exception
                  (lambda (a b) a))
                (exception 0 (+ "string" 5)))`), evalNormalProgram)).to.deep.equal(makeOk(0));
        // 3. Test with side effects
        expect(bind(parseL4(`
            (L4 (define f (lambda (x) (side_effects x) (newline) (+ x 1)))
                (define g (lambda (x y) 
                (if (= x 0)
                    2
                    y)))
                (g 0 (f 0)))`), evalNormalProgram)).to.deep.equal(makeOk(2));
        // 4. Test let
        expect(bind(parseL4(`
            (L4 (let ((x 1)) (+ x x)))`), evalNormalProgram)).to.deep.equal(makeOk(2));
    });
